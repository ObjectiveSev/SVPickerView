...
*/
private protocol AKCollectionViewLayoutDelegate: class {
...
private class AKCollectionViewLayout: UICollectionViewFlowLayout {
    ...
    fileprivate override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        if let delegate = delegate, let attributes = super.layoutAttributesForItem(at: indexPath)?.copy() as? UICollectionViewLayoutAttributes {
...
//        guard let frame = self.collectionView.layoutAttributesForItem(at: IndexPath(row: index, section: 0))?.frame else { return nil }
        
        guard let frame = collectionView.cellForItem(at: IndexPath(row: index, section: 0))?.frame else { return nil }
        
        let converted = collectionView.convert(frame, to: collectionView.superview)
        return convert(converted, to: superview)
    }
